...
function convertToLoopBody(ast: JuniperNode, variables: Record<string, object>, reveredVariables: Record<string, object>, currPath: string): JuniperNode {
  const replaced = { ...ast };

  if (replaced.name === 'interface' && replaced.value) {
    replaced.value = replaced.value.split('.').map((value, index) => {
      if (index === 0 && reveredVariables[value].toString() === 'interface.name') return `{{${reveredVariables[value]}}}`;
      if (index === 1 && reveredVariables[value].toString() === 'interface.unit.name') return `{{${reveredVariables[value]}}}`;
      return value;
    }).join('.');
  } else {
    ...
    if (typeof value === 'string') {
      replaced.value = `{{${currPath}}}`;
			...
			if (replaced.name) {
				replaced.name = reveredVariables[replaced.name] ? `{{${reveredVariables[replaced.name]}}}` : replaced.name;
			}

			if (replaced.value) {
				replaced.value = reveredVariables[replaced.value] ? `{{${reveredVariables[replaced.value]}}}` : replaced.value;
			}
		}
  }
  ...
  if (Array.isArray(replaced.children)) {
    replaced.children = replaced.children.map(child => convertToLoopBody(child, variables, reveredVariables, `${currPath}.${child.name}`));
  }
      ...
      if (index === subgroups[0]) {
        return [
          ...
          ...[
            ...
            convertToLoopBody(original[subgroups[0]], variables, flattenObjectReverse(variables), currPath.replace('interfaces', 'interface')),
            {
              type: 'flag',
              name: `{% endfor %}`,
              value: null,
